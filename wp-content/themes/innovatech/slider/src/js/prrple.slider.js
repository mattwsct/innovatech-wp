...
(function ($) {
  ...
  $.fn.prrpleSlider = function (params) {
    ...
    this.each(function () {
      ...
      $(this)
        .unbind()
        .data("prrpleSlider", new prrpleSliderCreate($(this), options));
    });
    return this;
  };
  ...
  $.fn.prrpleSliderSwipe = function (
    ...
    try {
      $(this)
        .data("prrpleSlider")
        .swipe(event, phase, direction, distance, orientation, callback);
    } catch (e) {}
  };
  ...
  function prrpleSliderCreate(root, options) {
    var s = {
      ...
      init: function () {
        if (options.debug)
          console.log("%c" + options.id + " init", "color:#0053A0");
        // transforms
        s.transforms =
          options.csstransforms != true ? false : s.test_transforms();
        // cloned
        s.cloned =
          options.transition == "slide" &&
          options.loop == true &&
          options.loopSeamless == true
            ? true
            : false;
        // get elements
        s.get_elements();
      ...
      get_elements: function () {
        if (options.debug)
          console.log("%c" + options.id + " get_elements", "color:#0053A0");
      ...
      get_info: function () {
        if (options.debug)
          console.log("%c" + options.id + " get_info", "color:#0053A0");
      ...
      update_class: function () {
        if (options.debug)
          console.log("%c" + options.id + " update_class", "color:#0053A0");
        // transition
      ...
      get_dims: function () {
        if (options.debug)
          console.log("%c" + options.id + " get_dims", "color:#0053A0");
        // reset
        ...
        if (options.height == null) {
          if (options.transition == "fade") {
            ...
            s.el.slides.css({
              width:
                (s.width / options.multiple) * (s.total + 2 * options.multiple),
            });
            if (options.direction == "vertical") {
              s.height = 0;
              s.slider.find(options.el_slide).each(function () {
      ...
      update_size: function () {
        if (options.debug)
          console.log("%c" + options.id + " update_size", "color:#0053A0");
        // slider
        if (options.width != null) {
          s.slider.css({
            width: s.width,
      ...
      update_visibility: function () {
        if (options.debug)
          console.log(
            "%c" + options.id + " update_visibility",
            "color:#0053A0"
          );
        if (options.transition == "fade") {
      ...
      add_clones: function () {
        if (options.debug)
          console.log("%c" + options.id + " add_clones", "color:#0053A0");
        // remove clones
        // s.slider.find(options.el_slide+'.cloned').remove();
        if (
          s.cloned == true &&
          s.slider.find(options.el_slide + ".cloned").length < 1 &&
          s.total > 1
        ) {
          var n;
          var slide;
          ...
          for (k = 0; k < clones2.length; k++) {
            clones2[k]
              .clone()
              .addClass("cloned cloned2")
              .prependTo(s.el.slides);
          }
          s.el.slide = s.slider.find(options.el_slide);
        }
      },
      ...
      add_dots: function () {
        if (options.debug)
          console.log("%c" + options.id + " add_dots", "color:#0053A0");
        if (s.total <= 1) {
          ...
          s.el.nav.find(options.el_dot).each(function () {
            ...
            $(this).click(function () {
              if (options.debug)
                console.log("%c--- click dot ---", "color:#0053A0");
              var slideNo = parseInt(
                $(this)
                  .attr("id")
                  .replace(options.el_dot.replace(".", "") + "_", "")
              );
              s.goTo(slideNo);
              return false;
            });
      ...
      add_arrows: function () {
        if (options.debug)
          console.log("%c" + options.id + " add_arrows", "color:#0053A0");
        if (s.total <= 1) {
          ...
          if (options.addArrows) {
            if (s.el.left.length < 1) {
              s.slider.append(
                '<a class="' + options.el_left.replace(".", "") + '">Prev</a>'
              );
            }
            if (s.el.right.length < 1) {
              s.slider.append(
                '<a class="' + options.el_right.replace(".", "") + '">Next</a>'
              );
            }
            s.el.left = s.slider.find(options.el_left);
            s.el.right = s.slider.find(options.el_right);
          ...
          if (s.el.left.length > 0) {
            ...
            s.el.right.unbind("click").click(function () {
              if (options.debug)
                console.log("%c--- click right ---", "color:#0053A0");
              s.slide_right();
              return false;
            });
            // left
            s.el.left.unbind("click").click(function () {
              if (options.debug)
                console.log("%c--- click left ---", "color:#0053A0");
              s.slide_left();
              return false;
            });
          }
      ...
      add_controls: function () {
        if (options.debug)
          console.log("%c" + options.id + " add_controls", "color:#0053A0");
        if (s.total <= 1) {
          s.el.controls.hide();
        } else {
          s.el.controls.show();
      ...
      hide_arrows: function () {
        if (options.debug)
          console.log("%c" + options.id + " hide_arrows", "color:#0053A0");
      ...
      autoplay: function () {
        if (options.debug)
          console.log("%c" + options.id + " autoplay", "color:#0053A0");
      ...
      get_pos: function (slide, direction) {
        if (options.debug)
          console.log("%c" + options.id + " get_pos", "color:#0053A0");
        var l =
          (direction == "vertical" ? s.height : s.width) / options.multiple;
        var total;
      ...
      get_pos_first: function (direction) {
        if (options.debug)
          console.log("%c" + options.id + " get_pos_first", "color:#0053A0");
        var total;
        if (s.cloned && s.total > 1) {
      ...
      get_pos_last: function (direction) {
        if (options.debug)
          console.log("%c" + options.id + " get_pos_last", "color:#0053A0");
        var l =
          (direction == "vertical" ? s.height : s.width) / options.multiple;
        var total;
        if (s.cloned && s.total > 1) {
        ...
        var total2 = parseInt(
          "-" + (total * l + parseInt(options.spacing) * total)
        );
        return total2;
      },
      ...
      get_pos_clone_first: function (direction) {
        if (options.debug)
          console.log(
            "%c" + options.id + " get_pos_clone_first",
            "color:#0053A0"
          );
        var l =
          (direction == "vertical" ? s.height : s.width) / options.multiple;
        ...
        var total2 =
          "-" + (total * l + parseInt(options.spacing) * (total - 1)) + "px";
        return total2;
      },
      ...
      get_pos_clone_last: function (direction) {
        if (options.debug)
          console.log(
            "%c" + options.id + " get_pos_clone_last",
            "color:#0053A0"
          );
        return "0px";
      },
      ...
      slide_left: function (skip_pause, swiping) {
        if (options.debug)
          console.log("%c" + options.id + " slide_left", "color:#0053A0");
      ...
      slide_right: function (skip_pause, swiping) {
        if (options.debug)
          console.log("%c" + options.id + " slide_right", "color:#0053A0");
      ...
      goTo: function (slideNo, skip, direction, swiping) {
        if (options.debug)
          console.log("%c" + options.id + " goTo " + slideNo, "color:#0053A0");
        // time
        var time;
        if (skip == true) {
          time = 0;
        ...
        s.el.nav
          .find("#" + options.el_dot.replace(".", "") + "_" + slideNo)
          .addClass("current");
        // animate slider
        if (options.transition == "fade") {
          // fade
          ...
          s.slider
            .find(options.el_slide + ":nth-child(" + slideNo + ")")
            .fadeIn(time);
        } else if (options.transition == "slide") {
          // slide
          if (options.debug)
            console.log(
              "%c" + options.id + " " + options.direction,
              "color:#0053A0"
            );
          // get position
          var dist;
          ...
          if (
            ...
            prev == s.total &&
            direction != "left" &&
            (s.total > 2 || typeof direction !== "undefined")
          ) {
          ...
          if (options.direction == "vertical") {
            ...
            if (s.transforms) {
              setTimeout(function () {
                // brief timeout avoids css animating reset
                if (skip == true) {
        ...
        add: function () {
          if (options.swiping == true) {
            if (typeof $.fn.swipe === "undefined") {
              console.log(
                "%cPlease include the jQuery TouchSwipe plugin for swipe gestures.",
                "color:#0053A0"
              );
            } else {
              if (
                options.richSwiping == true &&
                options.transition == "slide"
              ) {
                // rich swiping
                if (options.direction == "vertical") {
                  // vertical
                  s.slider.swipe({
                    swipeStatus: function swipeStatus(
                      event,
                      phase,
                      direction,
                      distance
            ...
            if (s.total > 1) {
              ...
              if (phase == "start") {
                ...
                if (
                  ...
                  orientation == "vertical" &&
                  direction == "up" &&
                  (s.current < s.total - options.multiple + 1 ||
                    s.cloned == true)
                ) {
                  // vertical down
      ...
      remove: function () {
        if (options.debug)
          console.log("%c" + options.id + " remove slider", "color:#0053A0");
        ...
        s.slider
          .find(options.el_slide)
          .removeClass("current current2 next next2 prev prev2");
      },
    };
